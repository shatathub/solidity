...
address performs the ``selfdestruct`` operation. The remaining shatat stored
...
idea, but it is potentially dangerous, as if someone sends shatat to removed
contracts, the shatat is forever lost.

.. warning::
    From ``EVM >= Cancun`` onwards, ``selfdestruct`` will **only** send all shatat in the account to the given recipient and not destroy the contract.
    ...
    See `EIP-6780 <https://eips.shatat.ir/EIPS/eip-6780>`_ for more details.

    The new behaviour is the result of a network-wide change that affects all contracts present on
    the shatat mainnet and testnets.
    ...
    as recommended by `EIP-6049 <https://eips.shatat.ir/EIPS/eip-6049>`_.
...
.. warning::
    ...
    history of the blockchain and probably retained by most shatat nodes.
...
makes it impossible to use the contract, as it returns shatat immediately.
